...
def _expand_target_id(
    target_id: str, bindings: Mapping[str, int], context: str
) -> str:
...
def _validate_field(field: FieldSpec, context: str) -> None:
    if not isinstance(field, (TextField, ImageField)):
        raise SvgSpecError(f"{context} must be a TextField or ImageField")
    ...
    if isinstance(field, ImageField):
        ...
        if field.default is not UNSET and field.default is not None and not isinstance(
            field.default, ImageSource
        ):
            raise SvgSpecError(f"{context}.default must be an ImageSource, None, or UNSET")
    elif field.default is not UNSET and not (
        field.default is None or isinstance(field.default, (str, int, float, bool))
    ):
        raise SvgSpecError(f"{context}.default must be a text value or UNSET")
    ...
    *,
    ...
    for position, definition in enumerate(definitions):
        ...
        if isinstance(definition, FixedSlots):
            if not isinstance(definition.capacity, int) or isinstance(
                definition.capacity, bool
            ) or definition.capacity <= 0:
                raise SvgSpecError(f"{here}.capacity must be a positive integer")
            if not isinstance(definition.min_items, int) or isinstance(
                definition.min_items, bool
            ) or not 0 <= definition.min_items <= definition.capacity:
                raise SvgSpecError(
                    f"{here}.min_items must be an integer between 0 and capacity"
                )
            if not isinstance(definition.index, str) or not _INDEX_RE.fullmatch(
                definition.index
            ):
            ...
            array_path = _join_path(context_path, definition.value_path, f"{here}.value_path")
        ...
        if callable(definition):
            if not slots:
                raise SvgSpecError(f"{here} field functions are only valid inside FixedSlots")
            ...
            if not isinstance(generated, (TextField, ImageField)):
                raise SvgSpecError(
                    f"{here} field function must return one TextField or ImageField"
                )
...
def _validate_length(value: str | None, target_id: str, name: str) -> None:
    ...
    try:
        ...
        raise SvgTemplateError(f"target id {target_id!r} has an invalid {name} length") from error
    if name in ("width", "height") and number <= 0:
        raise SvgTemplateError(f"target id {target_id!r} must have a positive {name} length")


def prepare_report(report: SvgReportTemplate) -> _PreparedReport:
    ...
    for item in expanded:
        ...
        if target.tag != _svg_tag("rect"):
            raise SvgTemplateError(f"target id {field.target_id!r} must identify a rect")
        if target.get("transform") is not None:
            raise SvgTemplateError(f"target id {field.target_id!r} must not have transform")
        missing = [name for name in ("x", "y", "width", "height") if target.get(name) is None]
        if missing:
            raise SvgTemplateError(f"target id {field.target_id!r} is missing {', '.join(missing)}")
    ...
    return _PreparedReport(tuple(expanded))


def _validate_array(root: Data, slot: _SlotRef) -> Sequence[Mapping[str, DataValue]]:
    try:
        value = _lookup(root, slot.array_path)
    except _PathMissing as error:
        raise SvgDataError(f"fixed slots path {_path_text(slot.array_path)!r} was not found") from error
    if not isinstance(value, (list, tuple)):
        raise SvgDataError(f"fixed slots path {_path_text(slot.array_path)!r} must resolve to a list or tuple")
    ...
    for item in value:
        if not isinstance(item, Mapping):
            raise SvgDataError(f"fixed slots path {_path_text(slot.array_path)!r} must contain mappings")
...
def _field_element(
    ...
    if isinstance(field, TextField):
        ...
        _validate_xml_characters(text, f"value_path {item.value_path!r} for {field.target_id!r}", SvgDataError)
        div.text = text
        ...
        ET.SubElement(
            ...
            _html_tag("img"),
            {
                ...
                "src": _image_data_uri(value, context=f"value_path {item.value_path!r} for {field.target_id!r}"),
...
def render_report(report: SvgReportTemplate, data: Data) -> str:
    if not isinstance(data, Mapping):
        raise SvgDataError("render data must be a mapping")
    prepared = report._prepared
    if not isinstance(prepared, _PreparedReport):  # pragma: no cover
    ...
    for item in prepared.fields:
        ...
        if active and item.field.target_stroke_path is not None:
            stroke = _target_paint_value(data, item, item.field.target_stroke_path, "stroke")
