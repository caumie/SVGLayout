...
def test_report_template_uses_fixed_slots_and_data_paint_paths() -> None:
    ...
    assert root.findall(f".//{{{SVG_NS}}}foreignObject")[1].get(
        "data-svgrb-value-path"
    ) == "items.1.name"
        ...
        SvgReportTemplate(
            ...
            fields=[
                FixedSlots(
                    ...
                    capacity=1,
                    index="row",
                    fields=[TextField(value_path="name", target_id="missing_{unknown}")],
                )
            ],
        )
...
def test_text_values_defaults_and_xml_characters_are_handled_generically() -> None:
    ...
    output = report.render(
        {"customer": {"name": "A & <B>"}, "values": {"0": 132000}}
    )
...
def test_optional_images_and_invalid_values_are_checked_by_the_library() -> None:
    ...
    assert _foreign_object(default_root, "image").find(
        f".//{{{XHTML_NS}}}img"
    ) is not None
    assert _foreign_object(none_root, "image").find(
        f".//{{{XHTML_NS}}}img"
    ) is None
